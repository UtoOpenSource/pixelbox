...
static uint8_t getcolor() {
	...
	color <<= 2;
	color |=
			color_gradient >= 0 ? color_gradient & 3 : randomNumber() & 3;
	return color;
}
...
int main() {
	...
	while (!WindowShouldClose()) {
		...
		int32_t x0 =
				(GetScreenToWorld2D((Vector2){0, 0}, cam).x) / CHUNK_WIDTH -
				1;
		int32_t x1 =
				...
				1;
		int32_t y0 =
				(GetScreenToWorld2D((Vector2){0, 0}, cam).y) / CHUNK_WIDTH -
				1;
		int32_t y1 =
		...
		DrawText(
				TextFormat("PIXELBOX %i FPS, %i FRAMETIME\n[NORMAL]:[CHUNK]\n\
CAM[%i, %i]\nCUR[%i, %i]:[%i, %i]\n",
									 (int)GetFPS(), (int)(GetFrameTime() * 1000),
									 (int)campos.x, (int)campos.y, (int)mousepos.x,
									 (int)mousepos.y, (int)mousepos.x / CHUNK_WIDTH,
