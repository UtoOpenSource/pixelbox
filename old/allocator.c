...
static void allocfree() {
	...
	while (node_list) {
		...
		if (f->count)
			fprintf(stderr,
							"ALLOC: leak detected! %i chunks are not freed!\n",
							f->count);
		else
...
struct chunk* allocChunk(int16_t x, int16_t y) {
	...
	// keep in mind, that all chunks are memsetted-zero by default and
	// after free!
...
static struct alloc_item* dataToNode(void* p,
																		 struct alloc_node** dst) {
...
void freeChunk(struct chunk* orig) {
	...
	n->count--;
	if (n->empty > it->index)
		n->empty = it->index;	 // set new empty node index
...
void debugAllocator(Rectangle rec) {
	...
	while (n) {
		for (int i = 0; i < NODE_LEN; i++) {
			...
			DrawPixel(rec.x + (x + b * 18) % (int)rec.width, rec.y + y,
								it->index != FREE_INDEX
										? Fade(BLUE, it->data.usagefactor /
																		 (float)CHUNK_USAGE_VALUE)
										: GRAY);
		}
