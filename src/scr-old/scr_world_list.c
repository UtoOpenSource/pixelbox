...
int GuiSavesViewEx(Rectangle bounds, struct save_file *saves,
									 int count, int *focus, int *scrollIndex,
									 int active);
...
static void create() {
	...
	for (size_t i = 0; i < files.count; i++) {
		...
		if (!getWorldInfo(files.paths[i], &saves[i].time,
											&saves[i].mode)) {
			saves[i].mode = -1;
			saves[i].time = 0;
		};
...
static void draw() {
	...
	if (saves && saves_len) {
		active =
				GuiSavesViewEx(rec, saves, saves_len, NULL, &scroll, active);
	} else {
		active = -1;
	...
	if (GuiButton(rec, "Open")) {
		SetWindowTitle(
				TextFormat("[pixelbox] : %s", saves[activez].path));
		openWorld(saves[activez].path);
		SetRootScreen(&ScrGamePlay);
	}
	GuiEnable();
	rec.y += 25;
	GuiLine(rec, TextFormat("%i : %s", active,
													saves ? saves[activez].path : "nul"));
}

static void update() {}

struct screen ScrWorldList = {NULL, draw, update, create, destroy};
