...
void freeToolkit() { GuiUnlock(); }

static uint8_t b_rainbow() { return rainbow_v; }

static uint8_t b_random() { return randomNumber(); }

static uint8_t b_noise() { return noise1(rainbow_v / 128.0) * 255; }
...
void setline(int x0, int y0, int x1, int y1) {
	...
	while (limit < 255) {
		limit++;
		setWorldPixel(x0, y0, getcolor(color_material, color_gradient),
									MODE_READ);
...
static Rectangle winrec = (Rectangle){0, 100, 400, 110};

static const char* tabs[] = {"world", "pallete", "stats", "tools",
														 NULL};
...
static Rectangle getbrec(int i, Rectangle rec) {
	return (Rectangle){rec.x + 2 + (i % (350 / 27)) * 25,
										 rec.y + 2 + (i / (350 / 27)) * 25, 25, 25};
}
...
static void colorbutton(Rectangle rec, int type) {
	...
	if (GuiColorButton(rec, getPixelColor(v), getbtext(type))) {
		color_material = type;
	};
	if (type == color_material) DrawRectangleLinesEx(rec, 2, YELLOW);
}

static Rectangle getgrec(int i, Rectangle rec) {
	return (Rectangle){rec.x + 12 + (12 + 2) * 25, rec.y + 2 + (i)*25,
										 25, 25};
}
...
static void gradbutton(Rectangle rec, int type) {
	...
	if (type == color_gradient) DrawRectangleLinesEx(rec, 2, GREEN);
}

static void drawPallete(Rectangle rec) {
	Rectangle item = (Rectangle){rec.x, rec.y, rec.width / 3, 25};
...
static void drawStats(Rectangle rec) {
	...
	GuiLabel(item,
					 TextFormat("CAM POS = %i:%i[in chunks=%i:%i]",
											(int)cam.target.x, (int)cam.target.y,
...
void drawToolkit() {
	...
	Rectangle recc =
			(Rectangle){// color rectangle
									winrec.x + winrec.width - 40, winrec.y + 2, 20, 20};
	...
	GuiColorButton(
			recc, getPixelColor(getcolor(color_material, color_gradient)),
			"");
	...
	switch (active_tab) {
		case 0: {	 // world
			if (GuiToggle(item, "Enable Physic", allowupdate) !=
					allowupdate) {
