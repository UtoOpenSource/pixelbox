...
int GuiTabBarEx(Rectangle bounds, int width, int closeable, const char **text, int count, int *active) {
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < bounds.x + bounds.width && tabBounds.x > bounds.x - tabBounds.width)
        {
            ...
            if (GuiToggle(tabBounds, text[i], toggle))
         			*active = i;
...
// Saves View control with extended parameters
int GuiSavesViewEx(Rectangle bounds, struct save_file *saves, int count, int *focus, int *scrollIndex, int active)
{
    ...
    // Draw visible items
    for (int i = 0; ((i < visibleItems) && (saves != NULL)); i++)
    {
				const char* text = NULL;
				if (saves[startIndex + i].mode >= 0 && 
						saves[startIndex + i].mode < world_modes_count) {
					text = TextFormat(
						"%s   (%i:%i:%i:%i) [%s]",
						saves[startIndex + i].path,
						saves[startIndex + i].time/60/60/24,
						(saves[startIndex + i].time/60/60)%24,
						(saves[startIndex + i].time/60)%60,
						(saves[startIndex + i].time)%60,
						world_modes[saves[startIndex + i].mode]
					);
				} else text = TextFormat(
						"%s   (CORRUPTED)",
						saves[startIndex + i].path
				);
        ...
        if (state == STATE_DISABLED)
        {
            ...
            GuiDrawText(text, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), Fade(GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_DISABLED)), guiAlpha));
        }
        else
        {
            if ((startIndex + i) == itemSelected)
            {
                ...
                GuiDrawText(text, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), Fade(GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_PRESSED)), guiAlpha));
            }
            else if ((startIndex + i) == itemFocused)
            {
                ...
                GuiDrawText(text, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), Fade(GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_FOCUSED)), guiAlpha));
            }
            else
            {
                // Draw item normal
                GuiDrawText(text, GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), Fade(GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_NORMAL)), guiAlpha));
            }
        }
