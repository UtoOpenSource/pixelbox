...
struct atoms* getChunkAtoms(struct chunk* c, const bool mode) {
	return c->buffers +
				 (mode == c->wIndex ? CHUNK_WIDTH * CHUNK_WIDTH : 0);
}
...
const char* world_modes[] = {"Normal", "Flat", "Sponge"};
...
static uint8_t (*generators[])(int32_t x, int32_t y) = {
		gen_normal, gen_flat, gen_sponge, NULL};
...
uint8_t softGenerate(int16_t ox, int16_t oy) {
	...
	if (World.mode < 0)
		...
		generator =
				World.mode < 3 ? generators[World.mode] : generators[1];
...
void generateChunk(struct chunk* c) {
	...
	for (int x = 0; x < CHUNK_WIDTH; x++) {
		for (int y = 0; y < CHUNK_WIDTH; y++) {
			data[x + y * CHUNK_WIDTH] =
					generator((int32_t)c->pos.axis[0] * CHUNK_WIDTH + x,
