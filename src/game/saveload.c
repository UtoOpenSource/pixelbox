...
static const char* init_sql =
		...
		"CREATE TABLE IF NOT EXISTS PROPERTIES (key STRING PRIMARY KEY, "
		"value);"
		"CREATE TABLE IF NOT EXISTS WCHUNKS (id INTEGER PRIMARY KEY, "
		"value BLOB);";
...
bool checkVersion(sqlite3* db, bool init) {
	...
	stmt = create_statement(
			db, "SELECT value FROM PROPERTIES WHERE key = 'version';");
...
void initSaveLoad(const char* path) {
	...
	char* msg = "no error";
	if (sqlite3_exec(World.database, init_sql, NULL, NULL, &msg) !=
			SQLITE_OK) {
...
void flushSaveLoad() {
	...
	if (sqlite3_exec(World.database, flush_sql, NULL, NULL, &msg) !=
			SQLITE_OK) {
...
int loadChunk(struct chunk* c) {
	if (World.database) {
		...
		sqlite3_stmt* stmt = create_statement(
				World.database, "SELECT value FROM WCHUNKS WHERE id = ?1;");
		if (!stmt) {
			return -2;
		}
...
void saveChunk(struct chunk* c) {
	if (World.database) {
		...
		sqlite3_bind_int64(stmt, 1, c->pos.pack);
		sqlite3_bind_blob(stmt, 2, getChunkAtoms(c, MODE_READ),
											sizeof(struct atoms), SQLITE_STATIC);
		while (statement_iterator(stmt) > 0) {
		}
		sqlite3_finalize(stmt);
	}
...
static bool getprop(sqlite3* db, const char* name, int64_t* out) {
	if (db) {
		bool loaded = false;
		sqlite3_stmt* stmt = create_statement(
				db, "SELECT value FROM PROPERTIES WHERE key = ?1;");
		if (!stmt) {
			perror(sqlite3_errmsg(db));
			return false;
		}
...
static bool setprop(sqlite3* db, const char* name, int64_t v) {
	if (db) {
		sqlite3_stmt* stmt = create_statement(
				db, "INSERT OR REPLACE INTO PROPERTIES VALUES(?1, ?2);");
		if (!stmt) {
			perror(sqlite3_errmsg(db));
			return false;
		}
