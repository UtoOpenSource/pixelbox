 ...
 *
 * This code is HIGHLY modified for pixelbox'es purposes!
 *
 * Example use:
 *
 * char cfgdir[256];
 ...
 */
static void get_config_folder_path(char *out, unsigned int maxlen, const char *appname)
{
	if (!out) return;
#ifdef CFGPATH_LINUX
 ...
 * @post The folder is created if needed.
 */
static void get_data_folder_path(char *out, unsigned int maxlen, const char *appname)
{
	if (!out) return;
#ifdef CFGPATH_LINUX
	const char *out_orig = out;
	...
	*out = 0;
	...
	get_config_folder_path(out, maxlen, appname);
#endif
}
 ...
 * @post The folder is created if needed.
 */
static void get_cache_folder_path(char *out, unsigned int maxlen, const char *appname)
{
	if (!out) return;
#ifdef CFGPATH_LINUX
	const char *out_orig = out;
...
std::string getConfigDir(const char* appname) {
	get_config_folder_path(buffer, MAX_PATH, appname);
	std::string res((char*)buffer);
	
	if (res.empty()) // fallback to current directory
		res = "." PATH_SEPARATOR_STRING;

	return res;
}

std::string getCacheDir(const char* appname) {
	get_cache_folder_path(buffer, MAX_PATH, appname);
	std::string res((char*)buffer);
	
	if (res.empty()) { // fallback
		res = "." PATH_SEPARATOR_STRING "cache" PATH_SEPARATOR_STRING;
		res += appname;
		res += PATH_SEPARATOR_STRING;
	}
...
std::string getDataDir(const char* appname) {
	get_data_folder_path(buffer, MAX_PATH, appname);
	std::string res((char*)buffer);
